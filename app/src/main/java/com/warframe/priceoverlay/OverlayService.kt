...
class OverlayService : Service() {
    ...
    private fun setRow(slug: String, name: String, state: RowState, price: String? = null) {
        rowState[slug] = state
        // Name on line 1, price on line 2 — prevents long names pushing price off screen
    ...
    private fun toggleLookupMode() {
        ...
        if (lookupModeActive) {
            ...
            tvLookupLabel.text = "● LOOKUP"; tvLookupLabel.setTextColor(Color.parseColor("#FF4488FF"))
            startLookupLoop()
        } else {
    ...
    private fun setupVirtualDisplay() {
        ...
        Log.d("WFOverlay", "screen=${screenWidth}x${screenHeight} statusBar=$statusBarHeight")
    ...
    private fun matchWithBounds(vt: com.google.mlkit.vision.text.Text, sf: Float, seen: MutableSet<String>) {
        for (block in vt.textBlocks) {
            ...
            val rect = Rect(
                (bb.left / sf).roundToInt(), (bb.top / sf).roundToInt(),
                (bb.right / sf).roundToInt(), (bb.bottom / sf).roundToInt()
            )
            ...
            for (text in candidates) {
                ...
                for (ws in 1..7) for (start in 0..words.size - ws) {
                    ...
                    if (match != null && seen.add(match.slug)) {
                        // Lookup mode: skip relics — only relic rewards should appear
